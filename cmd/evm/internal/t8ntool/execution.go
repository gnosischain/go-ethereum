...
func (pre *Prestate) Apply(vmConfig vm.Config, chainConfig *params.ChainConfig, txIt txIterator, miningReward int64) (*state.StateDB, *ExecutionResult, []byte, error) {
	...
	for i := 0; txIt.Next(); i++ {
		...
		receipt, bal, err := core.ApplyTransactionWithEVM(msg, gaspool, statedb, vmContext.BlockNumber, blockHash, pre.Env.Timestamp, tx, evm, nil)
		if err != nil {
...
// applyAuRaFinalization runs the AuRa consensus system calls: the block reward
// contract call and, for chains configured with one, the withdrawal contract
// call. Gnosis retains both after the merge, so fixtures generated by a Gnosis
// client expect them, whereas geth's generic t8n path runs no consensus engine.
func (pre *Prestate) applyAuRaFinalization(chainConfig *params.ChainConfig, statedb *state.StateDB, evm *vm.EVM) error {
	engine, err := aura.NewAuRa(chainConfig.Aura, rawdb.NewMemoryDatabase())
	if err != nil {
		return NewError(ErrorConfig, fmt.Errorf("failed constructing AuRa engine: %v", err))
	}
	defer engine.Close()
	...
	if err := engine.ApplyRewards(header, statedb, evm); err != nil {
		return NewError(ErrorEVM, fmt.Errorf("failed applying AuRa block reward: %v", err))
	}
	if pre.Env.Withdrawals == nil {
		return nil
	}
	if err := engine.ExecuteSystemWithdrawals(evm, pre.Env.Withdrawals); err != nil {
