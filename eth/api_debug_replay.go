...
func (api *DebugAPI) replayBuild(ctx context.Context, block *types.Block, statedb *state.StateDB, reverted []*rawdb.RevertedTx) (*bal.BlockAccessList, types.Receipts, uint64, common.Hash, error) {
	...
	applyReverted := func(tx *types.Transaction) {
		...
		_, _, err = core.ApplyTransactionWithEVM(ctx, msg, gp, statedb, header.Number, blockHash, header.Time, tx, evm, bc.Engine())
		if err == nil {
	...
	for k := 0; k <= len(committed); k++ {
		...
		statedb.SetTxContext(tx.Hash(), tcount, uint32(tcount+1))
		receipt, txBal, err := core.ApplyTransactionWithEVM(ctx, msg, gp, statedb, header.Number, blockHash, header.Time, tx, evm, bc.Engine())
		if err != nil {
	...
	bc.Engine().Finalize(bc, header, statedb, &body, receipts, evm, uint32(tcount+1), blockAL)
